...
- `SerialController.Core` — `ModbusSerialDeviceRegistry`: manages the dynamic (not hardcoded-count) list of configured devices, depends only on `Abstractions` (builds devices via the injected `IModbusSerialDeviceFactory`, never a concrete protocol implementation). Direct structural port of `AdamIoController.Core.AdamIoModuleRegistry`.
- `SerialController.Protocols.NModbus` — `ModbusSerialDevice`/`ModbusSerialDeviceFactory`, the Modbus RTU/ASCII implementation using the `NModbus` package; `SerialPortStreamResource`, a hand-written `IStreamResource` adapter NModbus itself doesn't ship for serial ports (see design decisions below); and `SerialPortConnection`/`SerialPortConnectionFactory`, a plain `ISerialPortConnection` wrapper over `System.IO.Ports.SerialPort` that's independent of the Modbus device (see design decisions for why both live in one project).
